...
function a3_pvc_include_style(){
	echo '<style type="text/css">.pvc_clear{clear:both} .pvc_stats{padding: 0px 5px 5px 0px !important;line-height: 10px;}</style>';
}
...
class A3_PVC
{
	...
	public static function pvc_stats_counter($post_id) {
		...
		if($results){
			$stats_html = '<p class="pvc_stats">' . number_format($results->total) . '&nbsp;' .__('lượt xem', 'pvc') /*. ', ' . number_format($results->today) . '&nbsp;' .__('views today', 'pvc') . '</p>'*/;
		}else{
